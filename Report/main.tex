...
\addcontentsline{toc}{subsection}{1.1 Correctness (6)}
...
(\texttt{conv2d\_taps}), im2col, and FFT implementations benchmarked in \S1.4.

\subsection*{1.2 Kernel rank (6)}
\addcontentsline{toc}{subsection}{1.2 Kernel rank (6)}
...
\addcontentsline{toc}{subsection}{1.3 Separability and low rank (10)}
...
\addcontentsline{toc}{subsection}{1.4 Runtime analysis (8)}
...
\addcontentsline{toc}{subsection}{2.1 Decomposition (5)}
...
\addcontentsline{toc}{subsection}{2.2 Hide and recover a payload (5)}
...
\addcontentsline{toc}{subsection}{2.3 Survive the channel (10)}
...
\begin{table}[H]
\centering
\resizebox{\textwidth}{!}{\input{tables_2_3_results.tex}}
...
\addcontentsline{toc}{subsection}{3.1 Recover two known transforms (4)}
...
\begin{table}[H]
\centering
\input{tables_3_1_breakpoints.tex}
...
\addcontentsline{toc}{subsection}{3.2 Choose the number of segments (3)}
...
\addcontentsline{toc}{subsection}{3.3 Identifiability (3)}
...
\addcontentsline{toc}{subsection}{4.1 Global equalisation (5)}
...
\addcontentsline{toc}{subsection}{4.2 Histogram matching and specification (6)}
...
\addcontentsline{toc}{subsection}{4.3 Local histogram processing (10)}
...
\addcontentsline{toc}{subsection}{4.4 Automatic correction (4)}
...
\paragraph{Footage and background plate.}
Clip: \emph{"Slap In The Face, Punch, Blow"} by KlausHausmann (Pixabay Content Licence,
credited in \texttt{ATTRIBUTION.md}), $1920\times1080$ at 25\,fps, 30.4\,s. It shows a subject
on an evenly-lit green backing reacting to an off-screen slap: real, unscripted motion (head
snap, recoil) produces genuine motion blur on several frames, fine loose hair with wispy
flyaway strands visible against the backing, and a second arm entering frame in a
green-tinted glove/sleeve showing heavy spill contamination -- all three of the handout's
required properties (hair, motion blur, translucent/soft-edged material) are present, none of
them staged for this assignment. 48 consecutive frames (the highest-scoring 48-frame window
found by \texttt{ingest\_greenscreen.py}, corresponding to source frames 276--323) were
extracted to $1280\times720$ and saved to \texttt{images/p5/frames/}. The clip is real
camera footage, not AI-generated or animated, and carries no watermark.

Background plate: \emph{"Wide view to Gornergletscher, Monte Rosa and Matterhorn, 2012
August"} by Ximonic (Simo Räsänen), CC BY-SA 3.0 (Wikimedia Commons, credited in
\texttt{ATTRIBUTION.md}), $9650\times3190$ px -- $7.5\times$ the $1280$px frame width, well
past the required $2.5\times$, giving ample room to pan.

\subsection*{5.1 Naive key (3)}
\addcontentsline{toc}{subsection}{5.1 Naive key (3)}

\paragraph{RGB dominance vs.\ chroma (hue angle).}
\texttt{key\_naive\_rgb} thresholds green dominance, $G-\max(R,B) > 60$. The chroma-only
keyer computes BT.601 YCbCr and thresholds the \emph{hue angle}
$\theta=\operatorname{atan2}(C_r-128,\,C_b-128)$ against the backing's own hue angle
(threshold $25^\circ$), rather than raw Euclidean $(C_b,C_r)$ distance: $\theta$ depends only
on the \emph{ratio} of $(R-Y)$ to $(B-Y)$, not their magnitude, so unlike dominance (an
absolute difference) or raw chroma distance (which shrinks toward the achromatic point as a
colour darkens, see below) it is exactly invariant to uniform brightness scaling --
demonstrated algebraically in the construction that follows.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Images/p5_5_1_naive_key.png}
\caption{Naive keys on a real frame. Backing here is evenly lit, so both keyers agree closely
on the silhouette (kept fraction $35.1\%$ vs.\ $33.0\%$); the hue-angle key is visibly noisier
on the desaturated jacket, where hue is close to undefined (small $|C_b-128|,|C_r-128|$ make
$\operatorname{atan2}$ unstable) -- the same caveat as the saturation threshold used for hue
statistics in \S4.2(c).}
\label{fig:p5_naive_key}
\end{figure}

\paragraph{Constructing the RGB failure.}
On this footage the backing is evenly lit, so the RGB failure is mild: dominance stays
$\ge65$ everywhere sampled, comfortably above the $60$ threshold used above. The handout
permits constructing the case where it would not be. Scaling the measured backing colour
$(61.6, 200.8, 70.2)$ toward black by a brightness factor $f$ simulates shadow/uneven
lighting on the \emph{same} backing hue:

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Images/p5_5_1_synthetic_darkening.png}
\caption{Same backing colour scaled by brightness factor $f$. RGB dominance (left) crosses
below threshold at $f\approx0.4$ (RGB $\approx(24,80,28)$), wrongly keeping shadowed backing
as foreground; hue angle (right) stays at exactly $0.00^\circ$ deviation at every $f$ tested.}
\label{fig:p5_darkening}
\end{figure}

At $f=0.4$ (RGB $=(24,80,28)$, a plausible shadowed patch of the same green backing),
dominance $=52.2 \le 60$: \texttt{key\_naive\_rgb} wrongly classifies this pixel as
foreground. The property responsible is that RGB dominance is an \emph{absolute} pixel-value
difference, and every RGB channel scales down together as a surface is more dimly lit, so
$G-\max(R,B)$ shrinks proportionally with brightness even though the hue has not changed at
all. Algebraically: for $Y=0.299R+0.587G+0.114B$, $C_b=128+0.564(B-Y)$,
$C_r=128+0.713(R-Y)$, scaling $(R,G,B)\to f(R,G,B)$ scales $(C_b-128)$ and $(C_r-128)$ by the
same factor $f$, leaving their \emph{ratio} -- and hence
$\theta=\operatorname{atan2}(C_r-128,C_b-128)$ -- exactly unchanged; this is confirmed by the
$0.00^\circ$ deviation at every $f$ in Figure~\ref{fig:p5_darkening}, down to $f=0.1$ where
dominance has collapsed to $13.1$.

\subsection*{5.2 Soft matte and spill suppression (7)}
\addcontentsline{toc}{subsection}{5.2 Soft matte and spill suppression (7)}

\texttt{estimate\_key\_colour} samples a 24-pixel border around the frame (assumed backing,
since a correctly framed subject does not touch the edges) and takes the \emph{median}
$(C_b,C_r)$, robust to any stray non-backing pixels at the border.

\paragraph{(a) Chroma-distance distributions.}
Chroma distance $d=\|(C_b,C_r)-\text{key}\|$ measured over three regions of one frame: pure
backing (top strip), pure foreground (face and jacket patches), and a hair-edge strip
expected to contain genuine partial coverage.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{Images/p5_5_2_chroma_distributions.png}
\caption{Chroma-distance distributions, three populations, one frame.}
\label{fig:p5_chroma_dist}
\end{figure}
...
\begin{tabular}{lcccc}
\toprule
Population & $n$ & mean & p1 & p99 \\
\midrule
Pure backing & 70{,}400 & 5.61 & 2.34 & 8.63 \\
Hair edge (partial) & 12{,}800 & 39.79 & 1.39 & 77.99 \\
Pure foreground & 63{,}000 & 74.04 & 43.79 & 90.80 \\
\bottomrule
\end{tabular}
\caption{Chroma-distance summary statistics per population.}
\end{table}

Backing and foreground \textbf{separate cleanly}: backing's p99 ($8.6$) is well below
foreground's p1 ($43.8$), with no overlap between the two populations' bulk. The hair-edge
population spans almost the entire gap between them (p1$=1.4$ to p99$=78.0$) rather than
clustering near either extreme, confirming genuine fractional optical mixing exists in this
footage, not just a sharp cutoff blurred by compression noise. This separation is what makes
a double-threshold soft key meaningful: $t_{\text{in}}=12$ (just above backing's p99) and
$t_{\text{out}}=58$ (at foreground's p1) were chosen directly from these percentiles, with a
small margin on each side.

\paragraph{(b) Continuous alpha and composite.}
\texttt{key\_soft} ramps linearly between $t_{\text{in}}$ and $t_{\text{out}}$. On this frame,
$1.06\%$ of pixels have fractional alpha ($0<\alpha<1$) --- small in absolute terms because
the subject's silhouette boundary is a thin curve relative to a $720\times1280$ frame, but
concentrated exactly where it matters (the hair strands, Figure~\ref{fig:p5_soft_composite}).

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Images/p5_5_2_soft_alpha_composite.png}
\caption{Soft alpha matte and composite against the panorama background plate. The matte
shows individual hair strands as grey (fractional alpha), not a hard cutoff.}
\label{fig:p5_soft_composite}
\end{figure}

\paragraph{(c) Spill suppression.}
\texttt{suppress\_spill} removes green excess $\max(G-\max(R,B),0)$ scaled by
$\text{strength}\times(1-\alpha)$. Scaling by $(1-\alpha)$ matters because $\alpha$ is itself
an estimate of how little backing light is mixed into a pixel: at $\alpha=1$ (confidently
pure foreground) any green excess is genuine subject colour and must not be touched; at
$\alpha=0$ the pixel is about to be replaced by the background pixel entirely, so correcting
its RGB is moot; spill contamination is concentrated exactly at fractional-alpha pixels, so
the correction should be strongest there and vanish at both extremes.

\begin{table}[H]
\centering
\begin{tabular}{lc}
\toprule
Strength & Mean residual green excess (fractional-alpha px) \\
\midrule
0 (before) & 57.93 \\
1.0 & 22.97 (60\% reduction) \\
2.0 & 8.77 (85\% reduction) \\
3.0 & 5.17 (91\% reduction) \\
5.0 & 2.74 (95\% reduction) \\
\bottomrule
\end{tabular}
\caption{Residual green excess vs.\ spill-suppression strength, fractional-alpha pixels only.}
\label{tab:p5_spill_strength}
\end{table}

At the hair edge, mean $\alpha\approx0.61$, so strength$=1$ removes only $\approx39\%$ of raw
excess by construction ($1-\alpha\approx0.39$) --- an intentional under-correction at
partially-opaque pixels, not a bug, but insufficient to fully clean visible fringing at this
particular alpha range. Strength$=3$ (91\% reduction) was used for the composite below.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Images/p5_5_2_spill_suppression.png}
\caption{Hair-edge crop before/after spill suppression (strength$=3$), and the green-excess
distribution restricted to fractional-alpha pixels, where spill actually lives (pure backing
is about to be discarded; pure foreground carries none).}
\label{fig:p5_spill_suppression}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{Images/p5_5_2_composite_spill_compare.png}
\caption{Composite with and without spill suppression, zoomed on the hair edge: visible
green/teal fringing on hair strands without suppression, substantially reduced with it. Some
residual fringing remains (Table~\ref{tab:p5_spill_strength}, 91\% not 100\% reduction),
consistent with strength$=3$ rather than full correction.}
\label{fig:p5_composite_spill}
\end{figure}

\paragraph{(d) Naive vs.\ soft matte comparison.}
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Images/p5_5_2_naive_vs_soft.png}
\caption{Naive hard matte, soft matte, and their absolute difference (MAD$=0.0026$ over the
whole frame).}
\label{fig:p5_naive_vs_soft}
\end{figure}
$\text{MAD}(\alpha_{\text{naive}},\alpha_{\text{soft}})=0.0026$ over the whole
$720\times1280$ frame, and gradient-field SAD $=228.32$ (raw units, \texttt{grad\_error}).
The two mattes \textbf{disagree sharply but only very locally}: the difference map
(Figure~\ref{fig:p5_naive_vs_soft}, right) is essentially zero everywhere except a thin outline
tracing the silhouette boundary plus a denser cluster exactly at the hair strands --- the same
region identified as genuinely fractional in (a). The aggregate MAD is small because agreement
is near-total over the $98.9\%$ of the frame that is unambiguously backing or unambiguously
foreground (both keyers agree there almost by definition, since a hard threshold and a soft
ramp converge far from the transition band); the disagreement is not spread thinly across the
whole frame but concentrated where the object being estimated --- alpha itself --- is
genuinely fractional and a single threshold cannot be right no matter where it is placed. The
gradient-field metric is the more informative of the two here: the hard matte's silhouette
boundary is a step (unbounded gradient magnitude at the edge, before smoothing) while the soft
matte's boundary is a ramp over a few pixels, so \texttt{grad\_error} captures exactly this
edge-sharpness difference that a per-pixel MAD, dominated by the vast agreeing interior,
underweights. The soft matte's gradient more faithfully predicts what is visible in the
composite (Figure~\ref{fig:p5_soft_composite}): individual hair strands with partial
translucency, which the naive matte's binary silhouette cannot represent at all.

\subsection*{5.3 Video, efficiently (5)}
\addcontentsline{toc}{subsection}{5.3 Video, efficiently (5)}

\paragraph{Vectorised panning composite.}
\texttt{ChromaLUT} precomputes a $128\times128$ $(C_b,C_r)$-quantised alpha table once (the
same double-threshold-ramp formula as \texttt{key\_soft}), so keying a frame costs one integer
division plus one fancy-index gather --- no per-pixel Python loop, and no repeated
distance computation for pixels that share a chroma bin. All 48 frames are keyed, panned, and
composited with whole-array NumPy operations (no per-pixel or per-frame Python loop beyond the
top-level list comprehension over 48 frames). The background crop advances linearly across the
full available pan range ($8370$px over 48 frames, background plate resized to frame height),
so the 48-frame sequence pans smoothly across the panorama rather than holding a static crop.

\begin{table}[H]
\centering
\begin{tabular}{lcc}
\toprule
Stage & Time (48 frames) & \% of total \\
\midrule
Keying (\texttt{ChromaLUT.alpha}) & 2.66\,s & 21\% \\
Background panning crop & $<$0.001\,s & 0\% \\
Composite + spill suppression & 4.81\,s & 37\% \\
mp4 encode (libx264) & 5.42\,s & 42\% \\
\midrule
Total & 12.89\,s & 100\% \\
\bottomrule
\end{tabular}
\caption{Per-stage timing, 48 frames at $1280\times720$.}
\label{tab:p5_timing}
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{Images/p5_5_3_timing_breakdown.png}
\caption{Pipeline timing breakdown.}
\label{fig:p5_timing}
\end{figure}

Processing throughput (keying + crop + composite, excluding encode) is $6.4$\,fps; end-to-end
including mp4 encoding is $3.7$\,fps. The \textbf{bottleneck is mp4 encoding} ($42\%$ of
total time), not the from-scratch image processing: \texttt{libx264} runs single-threaded
per-frame in this call pattern and is doing real per-frame entropy coding and motion-compensated
prediction, work with no NumPy-vectorised equivalent, whereas keying and compositing are both
already $O(HW)$ vectorised array operations completing in tens of milliseconds per frame.
Background cropping is free (a view/slice, no copy). Composite+spill is the largest processing
cost because \texttt{suppress\_spill} recomputes a full-frame \texttt{to\_ycbcr}-scale
elementwise excess map and \texttt{composite} allocates and blends two full float64 frames per
call --- both correct but not the leanest possible implementation (a fixed-point or in-place
version would reduce allocation, at the cost of readability).

\paragraph{Temporal flicker.}
This clip's studio backing is lit consistently enough that native frame-to-frame flicker,
measured in a static top-left corner patch (rows 10--60, cols 10--60, never covered by the
subject) across all 48 frames, is exactly $0$: every frame agrees on that patch's alpha to
floating-point precision. Demonstrating that a temporal filter reduces flicker therefore
required \emph{constructing} a case with flicker to remove, in the same spirit as \S5.1's
synthetic darkening: independent Gaussian chroma noise ($\sigma=12$, freshly seeded per frame,
simulating a noisier camera/low light) was added to each frame before keying.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Images/p5_5_3_flicker.png}
\caption{Left: static-region mean alpha, noisy vs.\ EMA-smoothed ($\gamma=0.35$). Right:
silhouette-edge column position (row $360$) across the pan/motion sequence, true edge (from
the clean, noise-free frames) vs.\ the edge recovered from noisy+EMA-smoothed alpha.}
\label{fig:p5_flicker}
\end{figure}

An exponential moving average, $\alpha_t^{\text{smooth}} = \gamma\alpha_t + (1-\gamma)
\alpha_{t-1}^{\text{smooth}}$ with $\gamma=0.35$, reduces the static-region \texttt{temporal\_flicker}
metric from $0.0455$ to $0.0145$ --- a $68.1\%$ reduction, visible in
Figure~\ref{fig:p5_flicker} (left) as the blue trace tracking a much smoother path than the
red. Critically, this filter does \textbf{not} smear the moving silhouette: comparing the
smoothed (noisy-input) edge position against the true edge position from the original
clean frames along a fixed row that crosses the subject's whole pan/motion arc
(Figure~\ref{fig:p5_flicker}, right), the mean absolute lag is only $1.98$px and the maximum is
$5.00$px, on a $1280$px-wide frame --- the filter suppresses frame-to-frame chroma noise
without perceptibly delaying genuine subject motion, because $\gamma=0.35$ still weights the
current frame more than half as much as the entire smoothed history combined.

Because this footage's native flicker is already zero, the delivered composite video
(\texttt{images/p5/composite\_raw.mp4}) uses the raw (unsmoothed) alpha from the clean frames
directly: applying EMA smoothing to already-flicker-free alpha would add unnecessary temporal
blur for no measurable benefit, so the filter is demonstrated on the constructed noisy case
above rather than applied to the final deliverable.
